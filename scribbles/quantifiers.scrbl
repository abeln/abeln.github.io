...
@html[lang: "en"]{
 ...
 @body{
  @article{
    @p{
       The following note defines a notion of @i{similarity} for @i{labelled-transition systems} (LTS) and shows how, by placing certain finiteness conditions
       on the LTS, similarity can be iteratively computed. None of what follows is novel, and is sourced from Davide Sangiorgi's excellent
    ...
    @p{
       An LTS is a triple \((Pr, Act, \rightarrow)\), where \(Pr\) is the set of @i{processes} (states), \(Act\) is a set of
    ...
    @p{A @i{simulation} is a relation \( R \subseteq Pr \times Pr \) such that if \( (P, Q) \in R \), then}
    @ul{
     @li{ for all \( P' \) such that \( P \rightarrow_{\mu} P'  \), there exists a \( Q' \) such that
      \( Q \rightarrow_{\mu} Q' \) and \( (P', Q') \in R \).}
    }
    @p{@i{Similarity}, denoted by \( \le \), is the union of all simulations; additionally, it too is a simulation.
